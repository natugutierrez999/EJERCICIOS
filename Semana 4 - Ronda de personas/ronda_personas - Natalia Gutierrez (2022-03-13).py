...
PERSONAS = int(input ("Ingrese cantidad de personas en ronda: ")) 
LIMITE_CUENTA = int(input ("Ingrese limite de cuenta: "))
...
if (LIMITE_CUENTA < PERSONAS):
    ...
    for ciclo in range(LIMITE_CUENTA):
        ...
        if (cuenta % 11 == 0): # cuenta es perfectamente divisible por 11
            if (giro == "horario"):
                persona = persona + 1 # se saltea 1 a contador de persona sentido horario
            else:
                persona = persona - 1 # se saltea 1 a contador de persona sentido antihorario
